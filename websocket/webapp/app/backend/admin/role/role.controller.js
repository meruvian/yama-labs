...
angular.module('websocketApp').controller('RoleCtrl', function ($scope, $modal, $location, Roles, angularPopupBoxes, ngstomp) {
	...
	ngstomp.subscribe('/topic/crud/Role', function(msg) {
		var a, role = JSON.parse(msg.body);

		angular.forEach($scope.roles, function(r) {
			if (r.id === role.id) {
				a = angular.extend(r, role);
			}
		});
