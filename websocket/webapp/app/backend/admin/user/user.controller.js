...
angular.module('websocketApp').controller('UserCtrl', function ($scope, $modal, $location, Users, angularPopupBoxes, ngstomp) {
	...
	ngstomp.subscribe('/topic/crud/User', function(msg) {
		var a, user = JSON.parse(msg.body);

		angular.forEach($scope.users, function(r) {
			if (r.id === user.id) {
				a = angular.extend(r, user);
			}
		});
